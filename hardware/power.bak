EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:open-automation
LIBS:smart-regulator-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 4 5
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L LM2576 U3
U 1 1 5936E607
P 8800 3675
F 0 "U3" H 9150 3875 60  0000 C CNN
F 1 "LM2576" H 8850 3475 60  0000 C CNN
F 2 "open-automation:Regulator-LM2576" H 9200 3625 60  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/texas-instruments/LM2576SX-5.0-NOPB/LM2576SX-5.0-NOPBCT-ND/3440106" H 9200 3625 60  0001 C CNN
F 4 "LM2576SX-5.0/NOPB" H 8800 3675 60  0001 C CNN "Part Number"
	1    8800 3675
	1    0    0    -1  
$EndComp
Text GLabel 8250 3375 1    60   Input ~ 0
V_MAIN
Text GLabel 9850 3425 1    60   Input ~ 0
V_USB
$Comp
L CP C1
U 1 1 593709F5
P 7900 3475
F 0 "C1" V 7950 3525 50  0000 L CNN
F 1 "100uF" V 7750 3375 50  0000 L CNN
F 2 "open-automation:CAP-RAD-5mm-UVR1H102MHD" H 7938 3325 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/wurth-electronics-inc/860240675007/732-9294-3-ND/5727524" H 7900 3475 50  0001 C CNN
F 4 "860240675007" H 7900 3475 60  0001 C CNN "Part Number"
	1    7900 3475
	0    1    1    0   
$EndComp
$Comp
L CP C2
U 1 1 5968D146
P 9900 3825
F 0 "C2" H 9925 3925 50  0000 L CNN
F 1 "1000uF" H 9925 3725 50  0000 L CNN
F 2 "open-automation:CAP-RAD-5mm-UVR1H102MHD" H 9938 3675 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/nichicon/UVR1H102MHD/493-1111-ND/588852" H 9900 3825 50  0001 C CNN
F 4 "UVR1H102MHD" H 9900 3825 60  0001 C CNN "Part Number"
	1    9900 3825
	1    0    0    -1  
$EndComp
$Comp
L L L1
U 1 1 5969017D
P 9650 3675
F 0 "L1" V 9600 3675 50  0001 C CNN
F 1 "47uH" V 9600 3675 50  0000 C CNN
F 2 "open-automation:L-RAD-7.4mm" H 9650 3675 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/abracon-llc/AIUR-06-470K/535-13031-ND/2343602" H 9650 3675 50  0001 C CNN
F 4 "AIUR-06-470K" V 9650 3675 10  0001 C CNN "Part Number"
	1    9650 3675
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR012
U 1 1 59690AC9
P 9900 4025
F 0 "#PWR012" H 9900 3775 50  0001 C CNN
F 1 "GND" H 9900 3875 50  0000 C CNN
F 2 "" H 9900 4025 50  0000 C CNN
F 3 "" H 9900 4025 50  0000 C CNN
	1    9900 4025
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR013
U 1 1 59690B3F
P 7700 3475
F 0 "#PWR013" H 7700 3225 50  0001 C CNN
F 1 "GND" V 7650 3300 50  0000 C CNN
F 2 "" H 7700 3475 50  0000 C CNN
F 3 "" H 7700 3475 50  0000 C CNN
	1    7700 3475
	0    1    1    0   
$EndComp
$Comp
L GND #PWR014
U 1 1 59690D86
P 9450 4075
F 0 "#PWR014" H 9450 3825 50  0001 C CNN
F 1 "GND" H 9450 3925 50  0000 C CNN
F 2 "" H 9450 4075 50  0000 C CNN
F 3 "" H 9450 4075 50  0000 C CNN
	1    9450 4075
	1    0    0    -1  
$EndComp
$Comp
L D_Schottky D1
U 1 1 59690FC6
P 9450 3875
F 0 "D1" H 9450 3975 50  0000 C CNN
F 1 "D_Schottky" H 9450 3775 50  0001 C CNN
F 2 "Diodes_SMD:D_SMA" H 9450 3875 50  0001 C CNN
F 3 "http://www.digikey.com/scripts/DkSearch/dksus.dll?Detail&itemSeq=233263850&uq=636356340886764109" H 9450 3875 50  0001 C CNN
F 4 "B330A-13-F" H 9450 3875 60  0001 C CNN "Part Number"
	1    9450 3875
	0    1    1    0   
$EndComp
$Comp
L R R5
U 1 1 59691BB3
P 4150 4725
F 0 "R5" V 4230 4725 50  0000 C CNN
F 1 "50m" V 4150 4725 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 4080 4725 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/susumu/RL1220T-R050-J/RL12T.05JCT-ND/567358" H 4150 4725 50  0001 C CNN
F 4 "RL1220T-R050-J" V 4150 4725 60  0001 C CNN "Part Number"
	1    4150 4725
	1    0    0    -1  
$EndComp
Text GLabel 3200 4525 0    60   Input ~ 0
BATTERY
Text GLabel 4075 4950 0    60   Input ~ 0
V_MAIN
$Comp
L CONN_01X02 BATTERY1
U 1 1 597573B9
P 5625 5775
F 0 "BATTERY1" H 5703 5816 50  0000 L CNN
F 1 "CONN_01X02" H 5703 5725 50  0000 L CNN
F 2 "open-automation:XT60" H 5625 5775 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/phoenix-contact/1934861/277-1607-ND/568644" H 5625 5775 50  0001 C CNN
	1    5625 5775
	1    0    0    -1  
$EndComp
Text GLabel 5325 5725 0    60   Input ~ 0
BATTERY
$Comp
L GND #PWR015
U 1 1 59757643
P 5375 5875
F 0 "#PWR015" H 5375 5625 50  0001 C CNN
F 1 "GND" H 5375 5725 50  0000 C CNN
F 2 "" H 5375 5875 50  0000 C CNN
F 3 "" H 5375 5875 50  0000 C CNN
	1    5375 5875
	1    0    0    -1  
$EndComp
Text GLabel 9425 4850 1    60   Input ~ 0
3V3
$Comp
L GND #PWR016
U 1 1 598E598C
P 8250 3825
F 0 "#PWR016" H 8250 3575 50  0001 C CNN
F 1 "GND" H 8250 3675 50  0000 C CNN
F 2 "" H 8250 3825 50  0000 C CNN
F 3 "" H 8250 3825 50  0000 C CNN
	1    8250 3825
	1    0    0    -1  
$EndComp
Text GLabel 7750 3675 0    60   Input ~ 0
power_EN
$Comp
L SPX1117M3-L-3-3/TR U11
U 1 1 598F6A23
P 8675 4950
F 0 "U11" H 8800 4750 50  0000 C CNN
F 1 "SPX1117M3-L-3-3/TR" H 8675 5200 50  0000 C CNN
F 2 "open-automation:SPX1117M3" H 8675 4550 50  0001 C CIN
F 3 "https://www.digikey.com/product-detail/en/exar-corporation/SPX1117M3-L-3-3-TR/1016-1848-1-ND/3586565" H 8725 4450 50  0001 C CNN
F 4 "1016-1848-1-ND" H 8675 4950 60  0001 C CNN "Part Number"
	1    8675 4950
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR017
U 1 1 598F6B7D
P 8225 5100
F 0 "#PWR017" H 8225 4850 50  0001 C CNN
F 1 "GND" H 8225 4950 50  0000 C CNN
F 2 "" H 8225 5100 50  0000 C CNN
F 3 "" H 8225 5100 50  0000 C CNN
	1    8225 5100
	1    0    0    -1  
$EndComp
Text GLabel 8175 4850 0    60   Input ~ 0
V_MAIN
$Comp
L C C3
U 1 1 598F70D3
P 9125 5150
F 0 "C3" H 9150 5250 50  0000 L CNN
F 1 "4.7uF" H 9150 5050 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603_HandSoldering" H 9163 5000 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/samsung-electro-mechanics-america-inc/CL10B475KQ8NQNC/1276-2087-1-ND/3890173" H 9125 5150 50  0001 C CNN
F 4 "CL10B475KQ8NQNC" H 9125 5150 60  0001 C CNN "Part Number"
	1    9125 5150
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR018
U 1 1 598F7252
P 9125 5400
F 0 "#PWR018" H 9125 5150 50  0001 C CNN
F 1 "GND" H 9125 5250 50  0000 C CNN
F 2 "" H 9125 5400 50  0000 C CNN
F 3 "" H 9125 5400 50  0000 C CNN
	1    9125 5400
	1    0    0    -1  
$EndComp
$Comp
L LED D4
U 1 1 598F7872
P 9525 5200
F 0 "D4" H 9525 5300 50  0000 C CNN
F 1 "LED" H 9525 5100 50  0000 C CNN
F 2 "Diodes_SMD:D_0603" H 9525 5200 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/rohm-semiconductor/SML-D12M8WT86/511-1578-1-ND/1641810" H 9525 5200 50  0001 C CNN
F 4 "SML-D12M8WT86" H 9525 5200 60  0001 C CNN "Part Number"
	1    9525 5200
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR019
U 1 1 598F7A99
P 9525 5750
F 0 "#PWR019" H 9525 5500 50  0001 C CNN
F 1 "GND" H 9525 5600 50  0000 C CNN
F 2 "" H 9525 5750 50  0000 C CNN
F 3 "" H 9525 5750 50  0000 C CNN
	1    9525 5750
	1    0    0    -1  
$EndComp
$Comp
L R R2
U 1 1 598F7B08
P 9525 5550
F 0 "R2" H 9575 5400 50  0000 C CNN
F 1 "1K" V 9525 5550 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 9455 5550 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/yageo/RC0603JR-071KL/311-1.0KGRTR-ND/726677" H 9525 5550 50  0001 C CNN
F 4 "RC0603JR-071KL" V 9525 5550 60  0001 C CNN "Part Number"
	1    9525 5550
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR020
U 1 1 59A20D2D
P 8000 4075
F 0 "#PWR020" H 8000 3825 50  0001 C CNN
F 1 "GND" H 8000 3925 50  0000 C CNN
F 2 "" H 8000 4075 50  0000 C CNN
F 3 "" H 8000 4075 50  0000 C CNN
	1    8000 4075
	1    0    0    -1  
$EndComp
Text GLabel 4500 4275 2    60   Input ~ 0
VCC_I2C
$Comp
L C_0.1uF C8
U 1 1 59C52B59
P 4275 4275
F 0 "C8" V 4350 4325 50  0000 L CNN
F 1 "C_0.1uF" V 4425 4125 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603_HandSoldering" H 4375 4575 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/kemet/C0603C104K5RACTU/399-5089-1-ND/1465623" H 4275 4025 50  0001 C CNN
F 4 "C0603C104K5RACTU" H 4400 4475 60  0001 C CNN "Part Number"
	1    4275 4275
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR021
U 1 1 59C53560
P 4075 4275
F 0 "#PWR021" H 4075 4025 50  0001 C CNN
F 1 "GND" V 4150 4200 50  0000 C CNN
F 2 "" H 4075 4275 50  0000 C CNN
F 3 "" H 4075 4275 50  0000 C CNN
	1    4075 4275
	0    1    1    0   
$EndComp
Wire Wire Line
	8000 4025 8000 4075
Connection ~ 9425 4950
Wire Wire Line
	9425 4850 9425 4950
Wire Wire Line
	9525 5700 9525 5750
Wire Wire Line
	9525 5350 9525 5400
Wire Wire Line
	9525 4950 9525 5050
Wire Wire Line
	9125 5400 9125 5300
Connection ~ 9125 4950
Wire Wire Line
	9125 5000 9125 4950
Wire Wire Line
	8175 4850 8275 4850
Wire Wire Line
	8225 5050 8275 5050
Wire Wire Line
	8225 5100 8225 5050
Wire Wire Line
	9025 4950 9525 4950
Wire Wire Line
	9450 3725 9450 3675
Connection ~ 9450 3675
Wire Wire Line
	9450 4075 9450 4025
Connection ~ 9850 3575
Wire Wire Line
	9900 4025 9900 3975
Connection ~ 9850 3675
Wire Wire Line
	9850 3425 9850 3675
Wire Wire Line
	9800 3675 9900 3675
Wire Wire Line
	9400 3675 9500 3675
Wire Wire Line
	9400 3575 9850 3575
Wire Wire Line
	8050 3475 8250 3475
Wire Wire Line
	8250 3575 8300 3575
Connection ~ 8250 3475
Wire Wire Line
	8250 3375 8250 3575
Wire Wire Line
	7700 3475 7750 3475
Wire Wire Line
	8250 3825 8250 3775
Wire Wire Line
	8250 3775 8300 3775
Wire Wire Line
	7750 3675 8300 3675
Wire Wire Line
	5375 5875 5375 5825
Wire Wire Line
	5375 5825 5425 5825
Wire Wire Line
	5325 5725 5425 5725
Wire Wire Line
	8000 3725 8000 3675
Connection ~ 8000 3675
Wire Wire Line
	4450 4275 4450 4575
Wire Wire Line
	4450 4575 4625 4575
Wire Wire Line
	4625 4675 4325 4675
Wire Wire Line
	4325 4675 4325 4525
Wire Wire Line
	4625 4775 4325 4775
Wire Wire Line
	4325 4775 4325 4950
Wire Wire Line
	4575 4900 4575 4875
Wire Wire Line
	4575 4875 4625 4875
Wire Wire Line
	4325 4950 4075 4950
Wire Wire Line
	4150 4950 4150 4875
Connection ~ 4150 4950
Wire Wire Line
	4325 4525 3750 4525
Wire Wire Line
	4150 4575 4150 4525
Connection ~ 4150 4525
Wire Wire Line
	4500 4275 4425 4275
Connection ~ 4450 4275
Wire Wire Line
	4075 4275 4125 4275
Wire Wire Line
	5550 4575 5700 4575
$Comp
L GND #PWR022
U 1 1 59C52714
P 4575 4900
F 0 "#PWR022" H 4575 4650 50  0001 C CNN
F 1 "GND" H 4575 4750 50  0000 C CNN
F 2 "" H 4575 4900 50  0000 C CNN
F 3 "" H 4575 4900 50  0000 C CNN
	1    4575 4900
	1    0    0    -1  
$EndComp
Text GLabel 5675 4775 2    60   Input ~ 0
SDA
Text GLabel 5675 4875 2    60   Input ~ 0
SCL
Wire Wire Line
	5675 4775 5550 4775
Wire Wire Line
	5675 4875 5550 4875
$Comp
L R R4
U 1 1 59C59874
P 3850 2700
F 0 "R4" V 3930 2700 50  0000 C CNN
F 1 "50m" V 3850 2700 50  0000 C CNN
F 2 "Resistors_SMD:R_0805_HandSoldering" V 3780 2700 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/susumu/RL1220T-R050-J/RL12T.05JCT-ND/567358" H 3850 2700 50  0001 C CNN
F 4 "RL1220T-R050-J" V 3850 2700 60  0001 C CNN "Part Number"
	1    3850 2700
	1    0    0    -1  
$EndComp
Text GLabel 3775 2500 0    60   Input ~ 0
V_PANEL
Text GLabel 3775 2925 0    60   Input ~ 0
V_MAIN
Text GLabel 4200 2250 2    60   Input ~ 0
VCC_I2C
$Comp
L C_0.1uF C7
U 1 1 59C59884
P 3975 2250
F 0 "C7" V 4050 2300 50  0000 L CNN
F 1 "C_0.1uF" V 4125 2100 50  0000 L CNN
F 2 "Capacitors_SMD:C_0603_HandSoldering" H 4075 2550 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/kemet/C0603C104K5RACTU/399-5089-1-ND/1465623" H 3975 2000 50  0001 C CNN
F 4 "C0603C104K5RACTU" H 4100 2450 60  0001 C CNN "Part Number"
	1    3975 2250
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR023
U 1 1 59C5988A
P 3775 2250
F 0 "#PWR023" H 3775 2000 50  0001 C CNN
F 1 "GND" V 3825 2100 50  0000 C CNN
F 2 "" H 3775 2250 50  0000 C CNN
F 3 "" H 3775 2250 50  0000 C CNN
	1    3775 2250
	0    1    1    0   
$EndComp
Wire Wire Line
	4150 2250 4150 2550
Wire Wire Line
	4150 2550 4325 2550
Wire Wire Line
	4325 2650 4025 2650
Wire Wire Line
	4025 2650 4025 2500
Wire Wire Line
	4325 2750 4025 2750
Wire Wire Line
	4025 2750 4025 2925
Wire Wire Line
	4275 2875 4275 2850
Wire Wire Line
	4275 2850 4325 2850
Wire Wire Line
	4025 2925 3775 2925
Wire Wire Line
	3850 2925 3850 2850
Connection ~ 3850 2925
Wire Wire Line
	4025 2500 3775 2500
Wire Wire Line
	3850 2550 3850 2500
Connection ~ 3850 2500
Wire Wire Line
	4200 2250 4125 2250
Connection ~ 4150 2250
Wire Wire Line
	3775 2250 3825 2250
$Comp
L GND #PWR024
U 1 1 59C598A4
P 4275 2875
F 0 "#PWR024" H 4275 2625 50  0001 C CNN
F 1 "GND" H 4275 2725 50  0000 C CNN
F 2 "" H 4275 2875 50  0000 C CNN
F 3 "" H 4275 2875 50  0000 C CNN
	1    4275 2875
	1    0    0    -1  
$EndComp
Text GLabel 5375 2750 2    60   Input ~ 0
SDA
Text GLabel 5375 2850 2    60   Input ~ 0
SCL
Wire Wire Line
	5375 2750 5250 2750
Wire Wire Line
	5375 2850 5250 2850
Wire Wire Line
	5250 2550 5325 2550
Wire Wire Line
	5325 2650 5250 2650
Connection ~ 5325 2600
Wire Wire Line
	5325 2550 5325 2650
Wire Wire Line
	5375 2600 5325 2600
$Comp
L GND #PWR025
U 1 1 59C599F5
P 5375 2600
F 0 "#PWR025" H 5375 2350 50  0001 C CNN
F 1 "GND" V 5425 2450 50  0000 C CNN
F 2 "" H 5375 2600 50  0000 C CNN
F 3 "" H 5375 2600 50  0000 C CNN
	1    5375 2600
	0    -1   -1   0   
$EndComp
$Comp
L CONN_01X02 PANEL1
U 1 1 59C5AB51
P 5650 6275
F 0 "PANEL1" H 5728 6316 50  0000 L CNN
F 1 "CONN_01X02" H 5728 6225 50  0000 L CNN
F 2 "open-automation:XT60" H 5650 6275 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/phoenix-contact/1934861/277-1607-ND/568644" H 5650 6275 50  0001 C CNN
	1    5650 6275
	1    0    0    -1  
$EndComp
Text GLabel 5350 6225 0    60   Input ~ 0
V_PANEL
$Comp
L GND #PWR026
U 1 1 59C5AB58
P 5400 6375
F 0 "#PWR026" H 5400 6125 50  0001 C CNN
F 1 "GND" H 5400 6225 50  0000 C CNN
F 2 "" H 5400 6375 50  0000 C CNN
F 3 "" H 5400 6375 50  0000 C CNN
	1    5400 6375
	1    0    0    -1  
$EndComp
Wire Wire Line
	5400 6375 5400 6325
Wire Wire Line
	5400 6325 5450 6325
Wire Wire Line
	5350 6225 5450 6225
$Comp
L Q_NMOS_GSD_PSMN038-100YLX Q1
U 1 1 59C5B7A9
P 3550 4425
F 0 "Q1" V 3450 4300 50  0000 L CNN
F 1 "Q_NMOS_GSD_PSMN038-100YLX" V 3775 3725 50  0000 L CNN
F 2 "open-automation:SOT-669_PSMN038-100YLX" H 3750 4525 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/nexperia-usa-inc/PSMN038-100YLX/1727-1815-1-ND/4813999" H 3750 4475 50  0001 C CNN
F 4 "PSMN038-100YLX" H 3850 4575 60  0001 C CNN "Part Number"
	1    3550 4425
	0    1    1    0   
$EndComp
Wire Wire Line
	3200 4525 3350 4525
$Comp
L R R3
U 1 1 59C5BE21
P 3550 3975
F 0 "R3" H 3600 3825 50  0000 C CNN
F 1 "1K" V 3550 3975 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 3480 3975 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/yageo/RC0603JR-071KL/311-1.0KGRTR-ND/726677" H 3550 3975 50  0001 C CNN
F 4 "RC0603JR-071KL" V 3550 3975 60  0001 C CNN "Part Number"
	1    3550 3975
	1    0    0    -1  
$EndComp
Text GLabel 3550 3725 1    60   Input ~ 0
3V3
Wire Wire Line
	3550 3725 3550 3825
Wire Wire Line
	3550 4125 3550 4225
Text GLabel 3400 4175 0    60   Input ~ 0
BAT_SW
Wire Wire Line
	3400 4175 3550 4175
Connection ~ 3550 4175
$Comp
L Power-Monitor-INA219 U5
U 1 1 59C5E21B
P 5100 4725
F 0 "U5" H 5300 4475 60  0000 C CNN
F 1 "Power-Monitor-INA219" H 5125 5025 60  0000 C CNN
F 2 "TO_SOT_Packages_SMD:SOT-23-8_Handsoldering" H 5200 4375 60  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/texas-instruments/INA219AIDCNR/296-23770-1-ND/1952550" H 5300 4475 60  0001 C CNN
F 4 "INA219AIDCNR" H 5400 4575 60  0001 C CNN "Part Number"
	1    5100 4725
	1    0    0    -1  
$EndComp
$Comp
L Power-Monitor-INA219 U2
U 1 1 59C5E60B
P 4800 2700
F 0 "U2" H 5000 2450 60  0000 C CNN
F 1 "Power-Monitor-INA219" H 4825 3000 60  0000 C CNN
F 2 "TO_SOT_Packages_SMD:SOT-23-8_Handsoldering" H 4900 2350 60  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/texas-instruments/INA219AIDCNR/296-23770-1-ND/1952550" H 5000 2450 60  0001 C CNN
F 4 "INA219AIDCNR" H 5100 2550 60  0001 C CNN "Part Number"
	1    4800 2700
	1    0    0    -1  
$EndComp
$Comp
L R_10k R8
U 1 1 59C5FC56
P 8000 3875
F 0 "R8" V 8000 3875 50  0000 C CNN
F 1 "R_10k" H 8025 3725 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 7930 3875 50  0001 C CNN
F 3 "https://www.digikey.com/product-detail/en/yageo/RC0603JR-0710KL/311-10KGRCT-ND/729647https://www.digikey.com/product-detail/en/yageo/RC0603JR-0710KL/311-10KGRCT-ND/729647" V 8080 3875 50  0001 C CNN
F 4 "RC0603JR-0710KL" V 8180 3975 60  0001 C CNN "Part Number"
	1    8000 3875
	1    0    0    -1  
$EndComp
Text GLabel 5650 4675 2    60   Input ~ 0
3V3
$Comp
L GND #PWR027
U 1 1 59C6263F
P 5700 4550
F 0 "#PWR027" H 5700 4300 50  0001 C CNN
F 1 "GND" H 5600 4550 50  0000 C CNN
F 2 "" H 5700 4550 50  0000 C CNN
F 3 "" H 5700 4550 50  0000 C CNN
	1    5700 4550
	-1   0    0    1   
$EndComp
Wire Wire Line
	5650 4675 5550 4675
Wire Wire Line
	5700 4575 5700 4550
$EndSCHEMATC
