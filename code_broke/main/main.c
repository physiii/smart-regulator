...
void init_protocols(void)
{
	...
	while (1) 
	{
		...
		if (!power_connected && token_connected) {
			wsi_power = NULL;
			i.pwsi = &wsi_power;
		...
		printf("\climate_connected %d  token_connected %d\n",climate_connected,token_connected);
		if (!climate_connected && token_connected) {
			wsi_climate = NULL;
			i.pwsi = &wsi_climate;
			i.protocol = "climate-protocol";
			i.path = "/climate";
			printf("\climate connecting\n");
			...
			/*
			if (initiate_protocols && token_connected) {
			wsi_buttons = NULL;
			i.pwsi = &wsi_buttons;
