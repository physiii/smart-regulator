...
#define POWER_DELAY_TIME     10 * 1000 /*!< delay time between different test items */
...
static void power_task(void* arg)
{
	...
	ret = INA219_measure_current( I2C_EXAMPLE_MASTER_NUM, &sensor_data_h, &sensor_data_l);
        ...
        if (ret == ESP_OK) {
	   battery_voltage = ( sensor_data_h << 8 | sensor_data_l ) * voltage_cf / 1000 - voltage_cb;
			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_CLIENT_WRITEABLE:
		printf("%s [LWS_CALLBACK_CLIENT_WRITEABLE]\n", tag);
		//break;
		...
		if (!power_linked) {
			...
			if (m < n)
			lwsl_err("error %d writing token request to power socket\n", n);
			else  {
				//printf("%s %s\n",tag,power_req_str);
			}
			break;
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
		...
		if (cJSON_GetObjectItem(root,"set_low_battery")) {
			cJSON *set_low_battery = cJSON_GetObjectItem(root,"set_low_battery");
			low_battery_on = cJSON_GetObjectItem(set_low_battery,"new_on")->valueint;
			low_battery_off = cJSON_GetObjectItem(set_low_battery,"new_off")->valueint;
			store_u32("low_battery_on",low_battery_on);
			store_u32("low_battery_off",low_battery_off);
		}

		if (cJSON_GetObjectItem(root,"set_high_battery")) {
			cJSON *set_high_battery = cJSON_GetObjectItem(root,"set_high_battery");
			high_battery_on = cJSON_GetObjectItem(set_high_battery,"new_on")->valueint;
			high_battery_off = cJSON_GetObjectItem(set_high_battery,"new_off")->valueint;
			store_u32("high_battery_on",high_battery_on);
			store_u32("high_battery_off",high_battery_off);
		}

		if (cJSON_GetObjectItem(root,"set_voltage")) {
			cJSON *set_voltage = cJSON_GetObjectItem(root,"set_voltage");
			voltage_cf = cJSON_GetObjectItem(set_voltage,"new_factor")->valueint;
			voltage_cb = cJSON_GetObjectItem(set_voltage,"new_bias")->valueint;
			store_u32("voltage_cf",voltage_cf);
			store_u32("voltage_cb",voltage_cb);
		}

		if (cJSON_GetObjectItem(root,"set_current")) {
			cJSON *set_current = cJSON_GetObjectItem(root,"set_current");
			current_cf = cJSON_GetObjectItem(set_current,"new_factor")->valueint;
			current_cb = cJSON_GetObjectItem(set_current,"new_bias")->valueint;
