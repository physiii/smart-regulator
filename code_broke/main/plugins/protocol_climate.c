...
#define CLIMATE_DELAY_TIME   5 * 1000 /*!< delay time between different test items */
...
static void climate_task(void* arg)
{
    ...
    tempature_cf = get_u32("tempature_cf",tempature_cf);
    tempature_cb = get_u32("tempature_cb",tempature_cb);
    humidity_cf = get_u32("humidity_cf",humidity_cf);
    humidity_cb = get_u32("humidity_cb",humidity_cb);
    light_cf = get_u32("light_cf",light_cf);
    light_cb = get_u32("light_cb",light_cb);
