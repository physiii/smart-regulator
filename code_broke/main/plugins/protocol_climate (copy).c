			...
			void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_PROTOCOL_INIT:
		...
		//xTaskCreate(power_task, "power_task", 1024*3, &vhd, 10, NULL);
                //xTaskCreate(power_task, "i2c_test_task_0", 1024 * 2, (void* ) 0, 10, NULL);
	...
	case LWS_CALLBACK_CLIENT_WRITEABLE:
		//printf("[LWS_CALLBACK_CLIENT_WRITEABLE] power_sum: %d\n",power_sum);
		if (!token_received) break;
		if (!power_linked) {
        	        strcpy(power_value_str, "{\"mac\":\"");
			strcat(power_value_str,mac_str);
        	        strcat(power_value_str, "\",\"device_type\":[\"regulator\"]");
        	        strcat(power_value_str, ",\"cmd\":\"link\"");
        	        strcat(power_value_str, ",\"token\":\"");
        	        strcat(power_value_str, token);
        	        strcat(power_value_str, "\"");
			strcat(power_value_str,"}");
			n = lws_snprintf((char *)p, sizeof(power_value_str) - LWS_PRE, "%s", power_value_str);
		...
		snprintf(temp_str, 10, "%d",power_sum);
                strcpy(power_value_str, "{\"mac\":\"");
		strcat(power_value_str,mac_str);
                strcat(power_value_str, "\",\"value\":");
                strcat(power_value_str, temp_str);
                strcat(power_value_str, ",\"device_type\":[\"room_sensor\"]");
                strcat(power_value_str, ",\"cmd\":\"power\"");
                strcat(power_value_str, ",\"token\":\"");
                strcat(power_value_str, token);
                strcat(power_value_str, "\"");
		strcat(power_value_str,"}");
